...
|--------|------|---------|-----------|-------------|-------------|
| BUG-001 | models.py (AlexNet) | Shape mismatch: expected 2048, got 3072 | Hardcoded classifier input size didn't match flattened feature dimensions (4×4×192=3072) | Changed `nn.Linear(2048, 1024)` to `nn.Linear(3072, 1024)` | N/A |
| BUG-002 | fit.py | Gradients explode across batches | Missing `optimizer.zero_grad()` accumulates gradients | Added `self.optimizer.zero_grad()` before `loss.backward()` | N/A |
| BUG-003 | train.py | Model learns at random-guessing level (~19% accuracy) | `drop_rate=0.99` disables 99% of neurons | Changed `drop_rate=0.99` to `drop_rate=0.5` | N/A |
| BUG-004 | fit.py | ValueError: target batch_size=0 on small batches | `.squeeze()` removes all size-1 dimensions, breaks on [1,1] → scalar | Changed `labels.squeeze()` to `labels.squeeze(1)` in both train & eval | N/A |
| BUG-005 | train.py, config.json | Wrong NUM_CLASSES configuration | Config hardcoded to 2, but cells data has 8 classes | Updated config with correct class counts per dataset | N/A |
| BUG-006 | models.py (AlexNet, VGG16, ResNet18) | Grayscale data (chest, orgs) crashes on hardcoded 3 channels | First Conv2d hardcoded `in_channels=3`, but grayscale is 1 | Made `in_channels` dynamic: `in_channels = kwargs.get("in_channels", 3)` | N/A |
| BUG-007 | models.py (AlexNet) | Last layer hardcoded to 11 classes | Output layer: `nn.Linear(1024, 11)` instead of `num_classes` | Changed to `nn.Linear(1024, kwargs.get("num_classes", 2))` | N/A |
